...
public class SefazGO_Entradas {

	public static void processarEntradas(Empresas empt, String mesAno, String competencia, TabelaProcessamento tb) throws IOException, AWTException {

		Pasta cp = new Pasta("NOTAS DE ENTRADA");
		...
		try {
			...
			if (VerificarTexto.naoEncontrado("Resultados!", pastaCompetencia)) {
				...
				Status status = new Status(empt, "SEM RESULTADOS", "NOTAS DE ENTRADA - SEFAZ GO");
			...
			if (VerificarTexto.naoEncontrado("Você não tem permissão", pastaCompetencia)) {
				...
				Status status = new Status(empt, "SEM PERMISSÃO", "NOTAS DE ENTRADA - SEFAZ GO");
				StatusDao sdDao = new StatusDao(status);
				sdDao.adicionarStatus();
			...
			if (VerificarTexto.naoEncontrado("Inscrição Estadual", pastaCompetencia)) {
				...
				Status status = new Status(empt, "ERRO DESCONHECIDO", "NOTAS DE ENTRADA - SEFAZ GO");
				StatusDao sdDao = new StatusDao(status);
				sdDao.adicionarStatus();
			...
			ProcessamentoEntrada.processarEntradas(pastaCompetencia, empt, tb, cp);
			
			ProcessamentoEntrada.deleteFile(caminho + ".zip");
